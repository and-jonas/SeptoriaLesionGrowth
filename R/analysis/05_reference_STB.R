...
list.of.packages <- c("tidyverse", "asreml", "SpATS", "car", "boot", "readxl")
...
# load data
data <- read_csv(paste0(data_path, "stb_reference_data.csv"))
# load experimental design
design <- read_csv(paste0(data_path, "meta/stb_design.csv"))
...
write.csv(genoblups, 
          paste0(out_data_path, "inc_blups.csv"),
          row.names = F)
...
write.csv(genoblups, 
          paste0(out_data_path, "sev_blups.csv"),
          row.names = F)
...
write.csv(pred, 
          paste0(out_data_path, "AUDPC_blups.csv"),
          row.names = F)
...
sev_blups <- read_csv(paste0(out_data_path, "sev_blups.csv")) %>% 
  dplyr::select(genotype_name, predicted.value) %>% 
  rename(pred_sev = predicted.value)
inc_blups <- read_csv(paste0(out_data_path, "inc_blups.csv")) %>% 
  dplyr::select(genotype_name, predicted.value) %>% 
  rename(pred_inc = predicted.value)
audpc_blups <- read_csv(paste0(out_data_path, "AUDPC_blups.csv")) %>% 
  rename(genotype_name = Gen_Name)
growth_blups <- read_csv(paste0(out_data_path, "growth_blups.csv")) %>% 
  dplyr::select(genotype_name, predicted.value) %>% 
  rename(pred_growth = predicted.value)
slope_blups <- read_csv(paste0(out_data_path, "slopes_blups.csv")) %>% 
...
png(paste0(figure_path, "growth_qr_corr.png"), width = 9, height = 3.5, units = 'in', res = 400)
plot(all)
dev.off()
...
p6 <- ggplot(pdat_all, aes(x = value, y = pred_sev)) +
  ...
  theme(plot.title = element_text(face = "bold"))
p6
png(paste0(figure_path, "slope_qr_corr.png"), width = 9, height = 3.5, units = 'in', res = 400)
plot(p6)
dev.off()
...
p7 <- ggplot(pdat_all, aes(x = value, y = pred_growth)) +
  ...
  theme(plot.title = element_text(face = "bold"))
p7
png(paste0(figure_path, "slope_main_corr.png"), width = 9, height = 3.5, units = 'in', res = 400)
plot(p7)
dev.off()
