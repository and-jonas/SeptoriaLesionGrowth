...
# save
saveRDS(qr_models4, paste0(out_data_path, "fits_area.rds"))

# load
qr_models4 <- readRDS(paste0(out_data_path, "fits_area.rds"))
...
# load intermediate results
pd <- readRDS(paste0(out_data_path, "subset_step2.rds")) %>% 
  extract_covars_from_nested("design", "genotype_name") %>% 
  mutate(batch_UID = paste(exp_UID, batch, sep = "_"))
...
write.csv(res, paste0(out_data_path, "meta.csv"), row.names = F)
